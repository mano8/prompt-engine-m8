...
- **Mutating verbs for template block membership (`C5`).** `POST /prompt-template/{id}/add-block/{block_id}/` and `PUT /prompt-template/{id}/set-block-position/{block_id}/`. Both operations change state and were previously reachable only by `GET`, which is cacheable, prefetchable and link-followable (`H3`). The `GET` forms remain mounted and are **marked deprecated in the OpenAPI document**; they delegate to the same handler and are scheduled for removal in the next minor. `DELETE .../delete-block/...` was already correct and is unchanged.
