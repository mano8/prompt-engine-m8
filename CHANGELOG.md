...
- **`requirements_prod.lock` regenerated onto the declared floors.** The
  shipped lock had lagged `requirements_base.txt` on two fronts: it still
  pinned `fastapi-m8 4.4.0` / `auth-sdk-m8 3.1.3` against the `>=4.5.1` floor
  raised in `2.2.0`, and the Dependabot floor bumps merged since (`uvicorn
  >=0.52.4`, `alembic >=1.19.2`, `redis >=8.1.0`) moved the declaration
  without moving the artifact — so `--require-hashes` kept installing a graph
  the floors no longer admit. Movement, all on declared packages: `fastapi-m8
  4.4.0 → 4.5.1`, `auth-sdk-m8 3.1.3 → 3.2.0` (transitive only, still
  undeclared), `uvicorn 0.49.0 → 0.52.4`, `alembic 1.18.5 → 1.20.0`, `redis
  8.0.1 → 8.1.0`. The other 44 pins are unchanged: the documented
  `pip-compile` command carries no `--upgrade`, so pins that still satisfy the
  graph are kept, which is the intended minimal-movement behaviour of a
  release lock. Regenerated on Linux (`python:3.14-slim`, the image's own base)
  so no host-only transitive such as `colorama` enters the lock. Verified the
  way `test-shipped-lock` does: hash-locked install, 49/49 pins matched after
  adding test tooling, 228 tests at 100% coverage, `pip-audit` clean.
- **`constraints.txt` / `constraints-all.txt` regenerated** for the same
  reason (`alembic 1.19.0 → 1.20.0`, `uvicorn 0.52.1 → 0.52.4`, dev-only
  `ruff 0.16.3 → 0.16.6`). Also produced on Linux this time, so `colorama`
  — a Windows-only `click` transitive that had entered both files from a
  Windows regeneration — drops out; they now describe the same platform as the
  lock and CI.
- pip-tools `7.6.1` (the first release that runs against pip `26.x`) renders
  a spurious `--no-index` into the autogenerated header of every compiled
  file. The header line in all three files is kept at the documented command;
  nothing was resolved offline.
